...
function tierInfo(t?: number) {
  return TIER_CFG[t ?? 0] ?? TIER_CFG[0];
}
...
@Component({
  ...
  template: `
    <div class="min-h-full bg-[#F8FAFF] text-[#1E3A5F] px-5 py-10 sm:px-8">
      <div class="max-w-[560px] mx-auto flex flex-col gap-6">
        <!-- ── HEADER ── -->
        <div class="animate-fade-up flex flex-col gap-2">
          <div
            class="inline-flex items-center gap-2 font-mono text-[.66rem]
                    tracking-[.14em] uppercase text-[#2563EB]"
          >
            <span class="w-[5px] h-[5px] rounded-full bg-[#2563EB]"></span>
            Ticket Verification
          </div>
          <h1
            class="font-bebas leading-none m-0"
        ...
        <!-- ── MODE TOGGLE ── -->
        <div
          class="flex gap-1.5 p-1 rounded-xl bg-white border border-[#E2E8F0]"
        >
          <button
        ...
        @if (mode() === 'manual') {
        <div
          class="animate-fade-up flex flex-col gap-4
                    bg-white border border-[#E2E8F0] rounded-[18px] p-5"
        >
          <div class="flex items-start gap-3">
            <div
              class="w-10 h-10 rounded-[10px] shrink-0 flex items-center justify-center
                        bg-[#2563EB]/8 border border-[#2563EB]/18 text-[#2563EB]"
            >
              <svg
                class="w-5 h-5"
                fill="none"
          ...
          <div class="flex gap-2.5 flex-col sm:flex-row">
            <input
              class="flex-1 bg-white border-[1.5px] rounded-[11px]
                     text-[#1E3A5F] font-mono text-[.82rem] tracking-[.04em]
            ...
            <button
              class="shrink-0 inline-flex items-center justify-center gap-2
                     px-5 py-3 rounded-[11px] bg-[#2563EB] text-white font-bold text-[.88rem]
                     transition-all duration-200
                     disabled:opacity-35 disabled:cursor-not-allowed
                     enabled:hover:shadow-[0_0_44px_rgba(37,99,235,.45)]
                     enabled:hover:-translate-y-px active:scale-95"
              [disabled]="
                state() === 'verifying' || !codeInput.trim() || inputErr()
              "
              (click)="verify(codeInput)"
            >
              @if (state() === 'verifying') {
              <span
                class="w-4 h-4 rounded-full border-2 border-[#E2E8F0] border-t-[#2563EB]
        ...
        <div class="animate-fade-up flex flex-col gap-4">
          ...
          <div class="flex flex-col gap-3">
            ...
            >
              <div class="relative shrink-0 w-11 h-11">
                ...
                <span
                  class="absolute inset-0 flex items-center justify-center
                                 font-mono text-[.7rem] font-bold text-slate-600"
                >
                  {{ timerSec() }}
                </span>
              </div>
              ...
              <div class="flex-1 min-w-0">
                <p class="text-[.82rem] font-semibold text-[#1E3A5F]">
                  Scanning…
                </p>
                <p
              ...
              <div class="shrink-0 flex items-center gap-1.5">
                ...
                ></span>
                <span
                  class="font-mono text-[.58rem] tracking-wide uppercase text-[#2563EB]"
                  >Live</span
                >
              </div>
            </div>
            ...
            <div class="flex items-center gap-3">
              ...
              <select
                class="flex-1 bg-white border border-[#E2E8F0] rounded-lg
                                   text-[#1E3A5F] text-[.82rem] px-3 py-2 outline-none"
          ...
          >
            <span
              class="w-5 h-5 rounded-full border-2 border-[#E2E8F0] border-t-[#2563EB]
                           shrink-0 animate-spin"
            ></span>
            <div>
              <p class="text-[.85rem] font-semibold text-[#1E3A5F]">
        ...
        <div
          class="animate-fade-up flex flex-col gap-4 rounded-[18px] p-5
                    bg-emerald-500/6 border border-emerald-500/22"
        >
          <div
            class="inline-flex items-center gap-2 w-fit px-3 py-1.5 rounded-full
          ...
          <div
            class="flex flex-col gap-2 bg-white border border-emerald-500/20 rounded-[14px] p-4"
          >
            @if (result()!.eventTicket?.ticketTemplate?.tier !== undefined) {
            <div
              class="inline-flex items-center gap-1 w-fit px-2.5 py-0.5 rounded-full border
                          font-mono text-[.62rem] tracking-[.08em] uppercase font-semibold"
              [style.color]="
                tierInfo(result()!.eventTicket!.ticketTemplate!.tier).color
              "
              [style.border-color]="
                tierInfo(result()!.eventTicket!.ticketTemplate!.tier).color +
                '44'
            ...
            <h2
              class="font-bebas text-[#1E3A5F] leading-tight m-0"
            ...
            <div class="flex flex-col gap-1.5 mt-1">
              ...
              <div class="flex items-center gap-2 text-[.78rem] text-slate-500">
                <svg
                  class="w-3.5 h-3.5 text-emerald-500/60 shrink-0"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
                  viewBox="0 0 24 24"
              ...
              } @if (result()!.eventTicket?.event?.city ||
              result()!.eventTicket?.event?.nameOfPlace) {
              <div class="flex items-center gap-2 text-[.78rem] text-slate-500">
                <svg
                  class="w-3.5 h-3.5 text-emerald-500/60 shrink-0"
                  fill="none"
            ...
            <div class="flex items-center justify-between gap-3 flex-wrap">
              <div>
                <p
                  class="font-mono text-[.52rem] tracking-[.14em] uppercase text-slate-400 mb-0.5"
                >
                  Ticket Code
                </p>
                <code
                  class="font-mono text-[.68rem] text-emerald-400 bg-emerald-500/8
                             px-2 py-0.5 rounded-md break-all"
                  >{{ result()!.ticketUniqueCode }}</code
                >
              </div>
              ...
              <div class="text-right">
                ...
                <span
                  class="font-mono text-[.9rem] font-semibold text-emerald-400"
                >
                  @if (result()!.eventTicket!.actualPrice === 0) { FREE } @else
                  { EGP
                  {{ result()!.eventTicket!.actualPrice | number : '1.0-2' }} }
                </span>
              </div>
              }
            </div>
          ...
          <button
            class="inline-flex items-center gap-2 w-fit px-4 py-2 rounded-[9px]
                         ...
                         hover:bg-emerald-500/8 transition-colors active:scale-95"
            (click)="reset()"
          >
            <svg
              class="w-3.5 h-3.5"
        ...
        <div
          class="animate-fade-up flex flex-col gap-4 rounded-[18px] p-5
                    bg-amber-500/6 border border-amber-500/22"
        >
          <div
            class="inline-flex items-center gap-2 w-fit px-3 py-1.5 rounded-full
                      font-mono text-[.72rem] font-semibold tracking-[.08em] uppercase
          ...
          <div
            class="flex flex-col gap-2.5 bg-white border border-amber-400/15 rounded-[12px] p-4"
          >
            <div class="flex justify-between gap-3 flex-wrap">
              ...
              >
              <span
                class="text-[.82rem] font-semibold text-[#1E3A5F] text-right"
                >{{ fmtDateTime(result()!.usedAt) }}</span
              >
            </div>
            @if (result()!.eventTicket?.event?.title) {
            <div class="flex justify-between gap-3 flex-wrap">
              ...
              <span
                class="text-[.82rem] font-semibold text-[#1E3A5F] text-right"
                >{{ result()!.eventTicket!.event!.title }}</span
              >
            </div>
            }
            <div class="flex justify-between gap-3 flex-wrap">
              ...
              >
              <code
                class="font-mono text-[.68rem] text-amber-400 bg-amber-400/8 px-2 py-0.5 rounded-md break-all"
              >
          ...
          <button
            class="inline-flex items-center gap-2 w-fit px-4 py-2 rounded-[9px]
                         ...
                         text-[.8rem] font-semibold hover:bg-amber-400/8
        ...
        <div
          class="animate-fade-up flex flex-col gap-4 rounded-[18px] p-5
                    bg-red-500/6 border border-red-500/22"
        >
          <div
            class="inline-flex items-center gap-2 w-fit px-3 py-1.5 rounded-full
                      font-mono text-[.72rem] font-semibold tracking-[.08em] uppercase
          ...
          <p class="text-[.88rem] font-semibold text-red-400/90 m-0">
            {{ errMsg() }}
          </p>
          <div class="flex justify-between gap-3 flex-wrap">
            <span
            ...
            >
            <code
              class="font-mono text-[.7rem] text-red-400 bg-red-500/8 px-2 py-0.5 rounded-md break-all"
            >
              {{ lastCode() }}
            </code>
          </div>
          <button
            class="inline-flex items-center gap-2 w-fit px-4 py-2 rounded-[9px]
                         ...
                         text-[.8rem] font-semibold hover:bg-red-500/8
                         transition-colors active:scale-95"
            (click)="reset()"
          >
            Try again
        ...
        <div
          class="animate-fade-up flex flex-col gap-4 rounded-[18px] p-5
                    bg-indigo-500/6 border border-indigo-500/22"
        >
          <div
            class="inline-flex items-center gap-2 w-fit px-3 py-1.5 rounded-full
                      font-mono text-[.72rem] font-semibold tracking-[.08em] uppercase
          ...
          <button
            class="inline-flex items-center gap-2 w-fit px-4 py-2 rounded-[9px]
                         ...
                         text-[.8rem] font-semibold hover:bg-indigo-400/8
                         transition-colors active:scale-95"
            (click)="reset()"
          >
            Try again
...
export class VerifyTicketComponent implements OnDestroy, AfterViewInit {
  ...
  private async startScanner(): Promise<void> {
    ...
    if (!QrScannerClass) {
      try {
        ...
        this.camErr.set(
          'qr-scanner library not installed. Run: npm install qr-scanner'
        );
        this.cdr.markForCheck();
        return;
      }
    ...
    try {
      ...
      this.cameras.set(
        cams.map((c: any) => ({ id: c.id, label: c.label || 'Camera' }))
      );
      ...
      .subscribe({
        ...
        error: (err: any) => {
          ...
          if (
            ...
            msg.toLowerCase().includes('event not')
          ) {
            this.errMsg.set(msg);
  ...
  async switchCamera(e: Event): Promise<void> {
    ...
    try {
      await this.scanner?.setCamera(id);
    } catch {}
  }
