      ...
      <div class="ei-card">
        ...
        <div class="ei-header">
          ...
          <div class="ei-title-row">
            <div>
              <h1 class="ei-title">
                Your <span class="ei-accent">Interests</span>
              </h1>
              <p class="ei-sub">
                Tap to add or remove genres from your personalised feed.
              </p>
            </div>
            @if (changed()) {
        ...
        <div class="ei-scroll">
          ...
          <div class="ei-error">
            ...
            <button class="ei-retry" type="button" (click)="load()">
              Retry
            </button>
          </div>
          } @else {
        ...
        <div class="ei-footer">
          <div class="ei-footer-hint" aria-live="polite">
            @if (selected().size === 0) { Select at least one genre } @else if
            (selected().size === 1) { 1 genre selected } @else {
            {{ selected().size }} genres selected }
          </div>
          ...
          >
            @if (saving()) {
            <span class="ei-spin"></span> Saving… } @else {
            <svg
              width="15"
              height="15"
              fill="none"
...
export class EditInterests implements OnInit {
  ...
  changed = computed(() => {
    ...
    for (const id of s) {
      if (!this.original.has(id)) return true;
    }
    return false;
  });
  ...
  save(): void {
    ...
    const ops: Array<
      ReturnType<typeof this.http.post | typeof this.http.delete>
    > = [];

    if (added.length > 0) {
      ops.push(
        this.http
          .post(`${this.baseUrl}/User/add-favorites`, added)
    ...
    removed.forEach((id) => {
      ops.push(
        this.http
          .delete(`${this.baseUrl}/User/remove-favorite/${id}`)
          .pipe(catchError(() => of(null)))
      );
    });
