...
function tierCfg(t: number): TierCfg {
...
function qrImgUrl(payload: string): string {
  return `https://api.qrserver.com/v1/create-qr-code/?size=240x240&color=000000&bgcolor=ffffff&qzone=2&data=${encodeURIComponent(
    payload
  )}`;
}
...
@Component({
  ...
  styles: [
    `
      @import url('https://fonts.googleapis.com/css2?family=Bebas+Neue&family=Plus+Jakarta+Sans:wght@300;400;500;600;700;800&family=DM+Mono:wght@400;500&display=swap');
      :host {
  ...
  template: `
    <div class="min-h-full bg-white text-[#0d2352]">
      <!-- ── HERO ── -->
      <header class="relative overflow-hidden px-6 pt-10 pb-8">
        <div
          class="pointer-events-none absolute -top-20 -right-16 w-72 h-72 rounded-full"
          style="background:radial-gradient(circle,rgba(37,99,235,.18) 0%,transparent 65%)"
        ></div>
        <div class="relative z-10">
          <div class="flex items-center gap-2 mb-3">
            <span
              class="font-mono text-[.58rem] tracking-[.12em] uppercase px-2.5 py-0.5 rounded-full
                         bg-blue-500/12 border border-blue-400/45 text-blue-700 font-bold"
              >My Tickets</span
            >
            @if (!loading() && bookings().length > 0) {
            <span
              class="font-mono text-[.58rem] tracking-[.12em] uppercase px-2.5 py-0.5 rounded-full
                           bg-slate-100 border border-[#E2E8F0] text-slate-600 font-bold"
            >
          ...
          <h1
            class="font-bebas leading-[.88] mb-1.5"
            style="font-size:clamp(3rem,9vw,5.5rem);letter-spacing:.04em"
          >
            <span
              class="block"
              style="-webkit-text-stroke:2px #2563eb;color:transparent"
              >YOUR</span
            >
            <span class="block text-[#0d2352]">BOOKINGS</span>
          </h1>
          <p class="text-sm text-slate-500 font-light">
            Tap "Show QR" on any ticket to display your entry code.
          </p>
        </div>
      </header>
      ...
      <main class="px-6 pb-12">
        ...
        <div class="flex flex-col gap-3">
          ...
          <div
            class="skeleton h-24 rounded-2xl border border-[#E2E8F0]"
        ...
        <div class="flex flex-col items-center gap-3 py-20 text-center">
          <div
            class="w-14 h-14 rounded-2xl bg-red-500/12 border border-red-400/35 flex items-center justify-center"
          >
            <svg
          ...
          <p class="font-bebas text-2xl tracking-[.04em] text-[#0d2352]">
            Couldn't load bookings
          </p>
          <button
            class="mt-1 px-5 py-2 rounded-xl bg-blue-500 text-white text-sm font-bold
        ...
        <div class="flex flex-col items-center gap-3 py-20 text-center">
          <div
            class="w-14 h-14 rounded-2xl bg-blue-500/12 border border-blue-400/35 flex items-center justify-center"
          >
            <svg
    ...
    <div
      ...
      style="background:rgba(13,35,82,.16);backdrop-filter:blur(16px);animation:backdropIn .2s ease both"
      (click)="closeQr()"
    >
      <!-- Modal sheet -->
      <div
        class="animate-modal-in w-full sm:max-w-[360px] rounded-t-3xl sm:rounded-3xl overflow-hidden
                    bg-white border border-[#E2E8F0] shadow-2xl"
      ...
      >
        ...
        <div class="flex items-start justify-between gap-3 px-5 pt-4 pb-2">
          <div class="flex-1 min-w-0">
            <span
              class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md border mb-1.5
                           font-mono text-[.56rem] tracking-[.1em] uppercase font-bold"
              [class]="
                tc(activeBooking()!).bg +
                ' ' +
                tc(activeBooking()!).border +
                ' ' +
                tc(activeBooking()!).text
              "
              [style.background]="tc(activeBooking()!).gradient"
            >
              {{ tc(activeBooking()!).label }}
            </span>
            <h2
              class="font-bebas text-[#0d2352] leading-tight truncate font-bold"
            ...
            <p
              class="font-mono text-[.6rem] tracking-wide text-slate-600 mt-0.5 font-medium"
            >
              {{ fmtDate(activeBooking()!.eventDate) }} ·
              {{ fmtTime(activeBooking()!.eventDate) }}
            </p>
            }
          </div>
          <button
            class="shrink-0 w-8 h-8 rounded-full bg-slate-100 border border-[#E2E8F0]
                           flex items-center justify-center text-slate-600
                           hover:bg-slate-200 hover:text-[#0d2352] transition-all active:scale-90"
            (click)="closeQr()"
          >
            <svg
              class="w-4 h-4"
        ...
        <div class="flex flex-col items-center gap-3 px-5 pb-6">
          <div
            class="w-[200px] h-[200px] rounded-2xl bg-white p-2 shadow-lg
                        flex items-center justify-center"
          >
            @if (!qrReady()) {
            <div class="flex flex-col items-center gap-2">
              <span
                class="w-7 h-7 rounded-full border-[3px] border-blue-200 border-t-blue-500
                               inline-block animate-spin"
              ></span>
              <span class="text-[.65rem] text-slate-600 font-mono font-medium"
                >Loading…</span
              >
            </div>
            <!-- Pre-load trigger -->
            <img
              [src]="qrImgUrl(activeBooking()!.qrPayload)"
              class="hidden"
              (load)="qrReady.set(true)"
              (error)="qrReady.set(true)"
              alt=""
            />
            } @else {
            <img
          ...
          <p
            class="font-mono text-[.6rem] tracking-[.14em] uppercase text-slate-600 font-bold"
          >
          ...
          <!-- Code display -->
          <div
            class="w-full bg-blue-500/8 border border-blue-400/35 rounded-xl px-4 py-3"
          >
            <div
              class="font-mono text-[.5rem] tracking-[.16em] uppercase text-slate-600 mb-1 font-bold"
            >
              Ticket Code
            </div>
            <div
              class="font-mono text-[.7rem] text-[#0d2352] break-all leading-relaxed tracking-wide font-medium"
            >
          ...
          <div class="w-full flex gap-2.5">
            <button
              class="flex-1 inline-flex items-center justify-center gap-1.5
                       py-2.5 rounded-xl text-[.78rem] font-bold border
            ...
            <a
              [href]="activeBooking()!.online_url"
              target="_blank"
              rel="noopener"
              class="flex-1 inline-flex items-center justify-center gap-1.5
                          py-2.5 rounded-xl text-[.78rem] font-bold
                          bg-blue-500/12 border border-blue-400/45 text-blue-600
                          hover:bg-blue-500/20 transition-colors active:scale-95"
            >
              <svg
                class="w-4 h-4"
                fill="none"
