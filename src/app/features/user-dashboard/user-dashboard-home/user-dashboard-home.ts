  ...
  template: `
    ...
    <div class="udh">
      ...
      <header class="udh-hero">
        <div class="udh-orb-a"></div>
        <div class="udh-orb-b"></div>
        <div class="udh-hero__body">
          ...
          <p class="udh-hero__sub">
            @if (loading()) { Loading your personalised feed… } @else if
            (totalEvents() > 0) {
            <strong class="udh-em">{{ totalEvents() }}</strong> events across
            <strong class="udh-em">{{ rows().length }}</strong>
            {{ rows().length === 1 ? 'genre' : 'genres' }} — curated for you. }
      ...
      <div class="udh-chips-wrap">
        <div class="udh-chips" role="toolbar" aria-label="Filter by genre">
          ...
          <button
            ...
            (mousedown)="startLP(cat.id, $event)"
            (mouseup)="stopLP()"
            (mouseleave)="stopLP()"
            (touchstart)="startLP(cat.id, $event)"
            (touchend)="stopLP()"
          >
            {{ cat.name }}
      ...
      <section class="udh-row">
        <div class="udh-row__head">
          <div
            class="udh-skel"
            style="width:3px;height:18px;border-radius:99px"
          ></div>
          <div
            class="udh-skel"
      ...
      <div class="udh-spotlight-wrap">
        <div class="udh-spotlight" (click)="open(spotlight()!)">
          ...
          <img
            [src]="spotlight()!.event_img_url"
            class="udh-spotlight__bg"
            [alt]="spotlight()!.title"
          />
          } @else {
          ...
          <div class="udh-spotlight__content">
            <div class="udh-spotlight__tags">
              <span class="udh-tag udh-tag--live"
                ><span class="udh-pulse"></span>Upcoming</span
              >
              @if (daysUntil(spotlight()!.start_time) !== null) {
              <span class="udh-tag udh-tag--dim"
                >In {{ daysUntil(spotlight()!.start_time) }}d</span
              >
              }
            </div>
      ...
      <div class="udh-section-hd">
        ...
        For You @if (activeFilter() !== null) {
        <button class="udh-clear" (click)="clearFilter()">Show all ×</button>
        }
      </div>

      @for (row of filteredRows(); track row.cat.id; let ri = $index) { @if
      (row.events.length > 0) {
      <section class="udh-row" [style.--rc]="row.color">
        <div class="udh-row__head">
          ...
          <div class="udh-row__btns">
            <button class="udh-row__btn" (click)="scrollQ(forYouQ, ri, -1)">
              ‹
            </button>
            <button class="udh-row__btn" (click)="scrollQ(forYouQ, ri, 1)">
              ›
        ...
        <div class="udh-row__track" #forYouTrack>
          ...
          <ng-container
            *ngTemplateOutlet="cardTpl; context: { ev, rc: row.color, i }"
          ></ng-container>
          }
        </div>
      </section>
      ...
      <section class="udh-row" style="--rc:#a78bfa;margin-bottom:2.5rem">
        <div class="udh-row__track udh-row__track--orgs">
          @if (orgsLoading()) { @for (n of [0,1,2,3,4]; track n) {
          <div
            class="udh-org-skel"
            [style.animation-delay]="n * 70 + 'ms'"
          ></div>
          } } @else { @for (org of discoveredOrgs(); track org.id; let i =
          $index) {
          <div class="udh-org-card" [style.animation-delay]="i * 55 + 'ms'">
            <div
              class="udh-org-card__av"
              [class.udh-org-card__av--default]="!org.logoUrl"
            >
              @if (org.logoUrl) {
              <img
                [src]="org.logoUrl"
                [alt]="org.name"
                class="udh-org-card__img"
              ...
              <div class="udh-org-card__default">
                ...
                <span class="udh-org-card__init-letter">{{
      ...
      <section class="udh-row" style="--rc:#a78bfa">
        ...
        <div class="udh-row__track">
          ...
          <ng-container
            *ngTemplateOutlet="cardTpl; context: { ev, rc: '#a78bfa', i }"
          ></ng-container>
          }
        </div>
      </section>
      ...
      <section class="udh-row" style="--rc:#FF4433">
        ...
        <div class="udh-row__track">
          ...
          <ng-container
            *ngTemplateOutlet="cardTpl; context: { ev, rc: '#FF4433', i }"
          ></ng-container>
          }
        </div>
      </section>
      ...
      <ng-template #cardTpl let-ev="ev" let-rc="rc" let-i="i">
        ...
        >
          <div class="udh-card__img-wrap">
            ...
            <span class="udh-card__dot"
              ><span class="udh-mini-pulse"></span
            ></span>
            }
          ...
          <div class="udh-card__body">
            ...
            <div class="udh-card__foot">
              ...
              <span class="udh-card__price">
                @if (minPrice(ev) === 0) { Free } @else { EGP
                {{ minPrice(ev) | number : '1.0-0' }} }
              </span>
              }
  ...
  styles: [
    `
      :host {
        ...
        --primary-brd: rgba(37, 99, 235, 0.2);
        --gold: #f59e0b;
      ...
      .udh-chip {
        ...
        border-radius: 100px;
        background: var(--cb, rgba(242, 238, 230, 0.06));
        border: 1.5px solid var(--cd, rgba(242, 238, 230, 0.1));
        color: var(--ct, var(--muted));
        font-family: 'Plus Jakarta Sans', sans-serif;
        font-size: 0.76rem;
        font-weight: 600;
        cursor: pointer;
      ...
      .udh-chip--active {
        ...
        box-shadow: 0 0 0 3px color-mix(in srgb, var(--ct) 18%, transparent);
      }
      .udh-chip--add {
        background: rgba(242, 238, 230, 0.04) !important;
        border-color: rgba(242, 238, 230, 0.1) !important;
        color: var(--muted) !important;
      }
      .udh-chip--add:hover {
        border-color: rgba(242, 238, 230, 0.22) !important;
      ...
      .udh-rm-toast {
        ...
        border: 1px solid rgba(255, 68, 51, 0.22);
        color: var(--coral);
        font-size: 0.75rem;
        font-weight: 500;
      ...
      .udh-section-hd--follow {
        color: #a78bfa;
      }
      .udh-section-hd--following {
        color: #a78bfa;
      }
      .udh-section-hd--trending {
        color: var(--coral);
      }
      .udh-section-hd__sub {
        font-size: 0.55rem;
      ...
      .udh-spotlight:hover .udh-spotlight__bg {
        transform: scale(1.05);
      }
      .udh-spotlight__bg--empty {
        background: var(--bg3);
      }
      ...
      .udh-spotlight__btn:hover {
        box-shadow: 0 0 44px rgba(240, 180, 41, 0.5);
      ...
      .udh-skel-hero {
        ...
        background: linear-gradient(
          90deg,
          rgba(242, 238, 230, 0.04) 25%,
          rgba(242, 238, 230, 0.07) 50%,
          rgba(242, 238, 230, 0.04) 75%
        );
      ...
      .udh-row__track {
        ...
        overflow-x: auto;
        overflow-y: visible;
        padding: 0.2rem 1.75rem 0.85rem;
        scrollbar-width: none;
        scroll-behavior: smooth;
        scroll-snap-type: x mandatory;
      }
      .udh-row__track::-webkit-scrollbar {
        display: none;
      }
      .udh-row__track--orgs {
        padding-top: 0.3rem;
      ...
      .udh-org-card__btn:hover {
        background: rgba(167, 139, 250, 0.12);
      }
      .udh-org-card__btn--on {
      ...
      .udh-card:hover .udh-card__arrow {
        opacity: 1;
...
export class UserDashboardHome implements OnInit, OnDestroy {
  ...
  totalEvents = computed(() =>
    this.rows().reduce((n, r) => n + r.events.length, 0)
  );
  ...
  removingName = computed(() => {
    ...
    return id !== null
      ? this.userCats().find((c) => c.id === id)?.name ?? ''
      : '';
  });
  ...
  chipBg(i: number) {
    return CHIP_BG[i % CHIP_BG.length];
  }
  chipBdr(i: number) {
    return CHIP_BDR[i % CHIP_BDR.length];
  }
  chipTxt(i: number) {
    return CHIP_TXT[i % CHIP_TXT.length];
  }
  todayLabel() {
  ...
  open(ev: EsEvent) {
    this.router.navigate(['/user-dashboard/events', ev.id]);
  }
  goEdit() {
    this.router.navigate(['/user-dashboard/edit-interests']);
  }

  setFilter(id: number) {
    if (!this.lpTimer) this.activeFilter.update((f) => (f === id ? null : id));
  }
  clearFilter() {
  ...
  scrollQ(ql: QueryList<ElementRef<HTMLDivElement>>, idx: number, dir: -1 | 1) {
    ql.toArray()[idx]?.nativeElement.scrollBy({
      left: dir * 650,
      behavior: 'smooth',
    });
  }
  ...
  private loadAll() {
    ...
    this.http
      .get<{ data: Category[]; success: boolean }>(
        `${this.BASE}/User/favorites`
      )
      .pipe(
        catchError(() => of({ data: [] as Category[], success: false })),
        ...
        if (cats.length === 0) {
          ...
          if (ids.length > 0) {
            this.catSvc
              ...
              .subscribe((all) =>
                this.fetchEventsForCats(all.filter((c) => ids.includes(c.id)))
              );
            return;
          }
        }
  ...
  private fetchEventsForCats(cats: Category[]) {
    if (cats.length === 0) {
      this.eventSvc
        .getAllEvents()
        .pipe(
          catchError(() => of([] as EsEvent[])),
          takeUntil(this.destroy$)
        )
        .subscribe((evs) => {
          this.rows.set([
            {
              cat: { id: -1, name: 'All Events' } as any,
              events: evs,
              color: ROW_COLORS[0],
            },
          ]);
    ...
    forkJoin(
      cats.map((c) =>
        this.http
          .get<{ data: any[] }>(
            `${this.BASE}/Event/Category?categoryId=${c.id}`
          )
        ...
        next: (results) => {
          ...
          const builtRows = cats.map((cat, i) => {
            ...
            const evs = normalizeEvents(results[i]?.data ?? []).sort((a, b) => {
              const d =
                (isUpcoming(b.start_time) ? 1 : 0) -
                (isUpcoming(a.start_time) ? 1 : 0);
              return d !== 0
                ? d
  ...
  private discoverOrgs(events: EsEvent[]) {
    ...
    events.forEach((ev) => {
      if (ev.organizationId) seenIds.add(ev.organizationId);
    });
    if (seenIds.size === 0) {
    ...
    forkJoin(
      ids.map((id) =>
        forkJoin({
          ...
          orgEvents: this.http
            .get<{ data: any[] }>(
              `${this.BASE}/Event/organization?organizationId=${id}`
            )
            .pipe(catchError(() => of({ data: [] as any[] }))),
        })
      )
      ...
      .subscribe((results) => {
        ...
        this.discoveredOrgs.set(
          orgs.sort((a, b) => {
            const diff =
              (this.followSvc.isFollowing(a.id) ? 1 : 0) -
  ...
  private loadTrending(excludeIds: number[]) {
    ...
    this.http
      .get<{ data: any[] }>(`${this.BASE}/Event/upcoming`)
      .pipe(
  ...
  private removeCategory(catId: number) {
    ...
    this.http
      .delete(`${this.BASE}/User/remove-favorite/${catId}`)
      .pipe(
        catchError(() => of(null)),
        takeUntil(this.destroy$)
      )
