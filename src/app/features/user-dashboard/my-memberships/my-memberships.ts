...
@Component({
  ...
  template: `
<div class="w-full min-h-full font-jakarta" style="background:#FFFFFF; color:#1E3A5F;">

  <!-- ══════════════  HEADER  ══════════════ -->
  <header class="relative overflow-hidden border-b border-[#E2E8F0]">
    <div class="absolute inset-0 pointer-events-none overflow-hidden">
      ...
      <div class="absolute rounded-full"
           style="width:50vw;height:50vw;top:0;right:-10%;
                  background:radial-gradient(circle,rgba(123,58,237,.05) 0%,transparent 65%);
                  filter:blur(80px)"></div>
    </div>

    <div class="relative z-10 mx-auto px-4 sm:px-8 lg:px-12 py-8 sm:py-10">
      <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p class="mt-2 text-sm text-slate-500 font-light max-w-sm leading-relaxed">
            Browse and join organisations, track your membership status, and manage your connections.
          </p>
        </div>
        @if (!loading() && !browseLoading()) {
          <span class="font-mono-dm text-[0.58rem] tracking-widest uppercase text-slate-400">
      ...
      @if (!loading() && myMemberships().length > 0) {
        <div class="grid grid-cols-3 gap-3 mt-6">
          @for (s of statsCards(); track s.label; let i = $index) {
            <div class="stat-card relative overflow-hidden rounded-2xl border border-[#E2E8F0]
                        bg-[#F8FAFF] p-3.5 cursor-default page-enter
                        transition-all duration-300 hover:-translate-y-1"
                 [style.animation-delay]="i * 60 + 'ms'">
              <div class="font-bebas text-3xl leading-none tracking-wider" [style.color]="s.color">
                {{ s.value }}
              </div>
              <div class="font-mono-dm text-[0.56rem] tracking-[.18em] uppercase text-slate-500 mt-1">
                {{ s.label }}
              </div>
  ...
  <!-- ══════════════  STICKY TAB BAR  ══════════════ -->
  <div class="sticky top-0 z-30 border-b border-[#E2E8F0]"
       style="background:rgba(255,255,255,.94);backdrop-filter:blur(20px)">
    <div class="mx-auto px-4 sm:px-8 lg:px-12">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between sm:gap-4">
        ...
        <div class="flex overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none]
                    [&::-webkit-scrollbar]:hidden border-b sm:border-b-0 border-[#E2E8F0] -mb-px">
          ...
          <button (click)="setTab('joined')"
                  class="font-mono-dm text-[0.6rem] tracking-[.12em] uppercase flex items-center
                         ...
                         hover:text-blue-600 transition-colors duration-150 whitespace-nowrap"
                  [class.text-blue-600]="activeTab()==='joined'"
                  [class.border-blue-600]="activeTab()==='joined'"
                  [class.text-slate-500]="activeTab()!=='joined'"
            ...
            <span class="inline-flex items-center justify-center min-w-[17px] h-[17px] px-1
                  ...
                  [class.bg-blue-600]="joinedCount()>0" [class.text-white]="joinedCount()>0"
                  [class.bg-slate-200]="joinedCount()===0" [class.text-slate-500]="joinedCount()===0">
              {{ joinedCount() }}
            </span>
          </button>
          ...
          <button (click)="setTab('pending')"
            ...
            <span class="inline-flex items-center justify-center min-w-[17px] h-[17px] px-1
                  ...
                  [class.bg-slate-200]="pendingCount()===0" [class.text-slate-500]="pendingCount()===0">
              {{ pendingCount() }}
            </span>
          </button>
          ...
          <button (click)="setTab('browse')"
                  class="font-mono-dm text-[0.6rem] tracking-[.12em] uppercase flex items-center
                         gap-1.5 px-3.5 py-2.5 border-b-2 shrink-0 bg-transparent cursor-pointer
                         hover:text-blue-600 transition-colors duration-150 whitespace-nowrap"
                  [class.text-blue-600]="activeTab()==='browse'"
                  [class.border-blue-600]="activeTab()==='browse'"
                  [class.text-slate-500]="activeTab()!=='browse'"
                  [class.border-transparent]="activeTab()!=='browse'">
            Browse All
            <span class="inline-flex items-center justify-center min-w-[17px] h-[17px] px-1
                         rounded-full text-[0.54rem] font-bold leading-none bg-blue-600 text-white">
              {{ allOrgs().length }}
            </span>
          </button>
        </div>
        ...
        <div class="relative flex items-center py-2 sm:w-52 sm:shrink-0">
          <input type="text"
                 ...
                 class="w-full py-2 pl-4 pr-10 rounded-full text-[13px] text-[#1E3A5F] outline-none
                        bg-white border border-[#E2E8F0] placeholder:text-[#64748B]
                        focus:border-blue-400/40 focus:ring-2 focus:ring-blue-400/10
                        transition-all duration-150"/>
          <svg class="absolute right-3.5 w-3.5 h-3.5 text-slate-400 pointer-events-none"
  ...
  <!-- ══════════════  MAIN CONTENT  ══════════════ -->
  <main class=" mx-auto px-4 sm:px-8 lg:px-12 py-6 sm:py-8">

    <!-- Loading -->
    @if (loading() || (activeTab()==='browse' && browseLoading())) {
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        @for (i of [1,2,3,4,5,6,7,8]; track i) {
          <div class="skeleton rounded-2xl border border-[#E2E8F0] h-52"
    ...
    <!-- ── JOINED / PENDING TABS ── -->
      ...
      @if (filteredMyOrgs().length === 0) {
        <div class="flex flex-col items-center justify-center py-24 text-center gap-5">
          ...
          <div>
            <p class="font-bebas text-2xl tracking-widest text-slate-700">{{ emptyLabel() }}</p>
            <p class="mt-1 text-[0.84rem] text-slate-500 max-w-xs mx-auto leading-relaxed">
              {{ emptySubLabel() }}
            </p>
          </div>
          @if (activeTab()==='joined') {
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          @for (m of filteredMyOrgs(); track m.organizationId; let i = $index) {
            <div class="org-card relative rounded-2xl overflow-hidden flex flex-col page-enter
                        border bg-[#F8FAFF]
                        transition-all duration-300 hover:-translate-y-1
                        hover:shadow-[0_20px_60px_rgba(37,99,235,.1)]"
                 [class.border-amber-400/20]="m.status===MS.Pending"
                 [class.hover:border-amber-400/35]="m.status===MS.Pending"
                 [class.border-[#E2E8F0]]="m.status!==MS.Pending"
                 [class.hover:border-blue-400/25]="m.status!==MS.Pending"
              ...
              <div class="px-4 pb-4 flex flex-col gap-3 flex-1">
                ...
                @if (m.organization?.bio) {
                  <p class="text-[0.78rem] text-slate-600 leading-relaxed line-clamp-2">
                    {{ m.organization!.bio }}
                  </p>
                }
                ...
                <div>
                  ...
                  <p class="mt-1.5 font-mono-dm text-[0.54rem] tracking-wider text-slate-400">
...
export class MyMembershipsPage implements OnInit {
  ...
  joinedCount = computed(
    () =>
      this.myMemberships().filter((m) => m.status === MembershipStatus.Approved)
        .length
  );
  pendingCount = computed(
    () =>
      this.myMemberships().filter((m) => m.status === MembershipStatus.Pending)
        .length
  );
  ...
  join(org: OrgSummary) {
    ...
    this.svc.joinOrganization(org.id).subscribe({
      ...
      error: (err) => {
        toast.error(err.error?.message || 'Failed to send request');
        this.busyId.set(null);
      },
    });
  }
  ...
  leave(orgId: number, orgName: string) {
    ...
    this.svc.leaveOrganization(orgId).subscribe({
      next: () => {
        this.myMemberships.update((l) =>
          l.filter((m) => m.organizationId !== orgId)
        );
        toast.success('You left ' + orgName);
        this.busyId.set(null);
      },
      error: (err) => {
        toast.error(err.error?.message || 'Failed to leave');
        this.busyId.set(null);
      },
    });
  }
  ...
  getLogoStyleFromMembership(m: UserOrgMembership): string {
    return (
      'background:' + this._palettes[m.organizationId % this._palettes.length]
    );
  }
